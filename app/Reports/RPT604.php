...
class RPT604 extends ReportDataAbstract
{
    protected $inData;
    ...
    protected function getRows()
    {
        ...
        foreach ($jdoc_array as $element) {
            ...
            $result[$element->jdoc->vehicle->licPlateState][] = $element;
        }
        ...
        return $rows;
        // $startdate = $this->inData['endDate'];
        // $endDate = $this->inData['endDate'];


        // $users  = DB::select("SELECT COUNT(id), jdoc->'vehicle'->>'licPlateState' as state FROM users where client_id = $client->id AND jdoc->'vehicle'->>'licPlateState' != '' GROUP BY state;");
