...
<script>
    ...
    import UserDetail from './ManagerAdminUserDetail.vue'
    import Config from '../../config'
    let formHelper = require ('../../helpers/form.js').default

    export default {
        name: 'manage-admin-user-add',
        data () {
            return {
                addFormSave: true,
                roleOptions: [
        ...
        methods: {
            formComplete: function (user, response) {
                if(response.status == 200) {
                    ...
                    this.$router.push({name: 'manageuser-employees', params: {uniq_id: this.$route.params.uniq_id}})
                }
            },
        }
    }
