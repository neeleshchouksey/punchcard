...
<template>
    <div>
        ...
        <api-form
            id="user-add-form" method="POST" action="/webapp/users"
            v-model="defaultUser" v-on:form-complete="formComplete">

            <input type="hidden" v-if="defaultUser.client_uniq_id" v-model="defaultUser.client_uniq_id" />

            <user-detail v-if="defaultUser"
                v-on:input="validateInput" v-model="defaultUser"
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                addFormSave: true,
                roleOptions: Config.roleOptions('addingToSelf'),
