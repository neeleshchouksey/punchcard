    ...
    export default {
        ...
        created () {
            ...
            this.setReportOptions({
                'showUsers': false,
                'showProjects': false,
                'showClients': Perm.isA('superadmin'),
                'showDates': true,
                'required': [],
            });
        },
        ...
        computed: {
            ...
            computedRows: function ()
            {
                ...
                let exportableReportData = _.map(this.reportData.reportData.rows, (row) => {
                    ...
                    row[1] = '<a target="_blank" href="/project/' + row[0] + '/summary">' + project_name + '</a>';
                    row[5] = '<a target="_blank" href="/project/' + row[0] + '/users">Employees</a>';
                    row[6] = '<a target="_blank" href="/project/' + row[0] + '/clients">Companies</a>';
                    return row;
                });
